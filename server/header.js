...
class DynamicHeader {
    ...
    clearDynamicItems(nav) {
        const items = nav.querySelectorAll('.nav-item:not(:first-child), .sidebar__nav-item');
    ...
    addMainNavItem(nav, category) {
        ...
        if (category.subcategories && category.subcategories.length > 0) {
            ...
            li.innerHTML = `
                <a class="nav-link" href="/category/${category.nameEnglish.toLowerCase()}/" role="button">
                    ${category.name}
            ...
            li.innerHTML = `
                <a class="nav-link" href="/category/${category.nameEnglish.toLowerCase()}/">
                    ${category.name}
                </a>
            `;
        }
    ...
    addSidebarNavItem(nav, category) {
        ...
        if (category.subcategories && category.subcategories.length > 0) {
            ...
            li.innerHTML = `
                <a class="sidebar__nav-link" href="/category/${category.nameEnglish.toLowerCase()}/" role="button">
                    ${category.name}
            ...
            li.innerHTML = `
                <a class="sidebar__nav-link" href="/category/${category.nameEnglish.toLowerCase()}/">
                    ${category.name}
                </a>
            `;
        }
